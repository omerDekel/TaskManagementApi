...
namespace TaskManagementApi.Tests
{
    public class TaskRepositoryTests
    {
        ...
        public async Task AddAsync_ShouldAddTask()
        {
            ...
            var task = new SqlToDoTask { Id = 1, Title = "Test Task" };
            _mapperMock.Setup(m => m.Map<SqlToDoTask>(taskDto)).Returns(task);
        ...
        {
            _context.Tasks.Add(new SqlToDoTask { Id = 3, Title = "Task 1" });
            _context.Tasks.Add(new SqlToDoTask { Id = 4, Title = "Task 2" });
            await _context.SaveChangesAsync();
            ...
            _mapperMock.Setup(mapper => mapper.Map<IEnumerable<ToDoTaskDto>>(It.IsAny<IEnumerable<SqlToDoTask>>())).Returns(taskDtos);
